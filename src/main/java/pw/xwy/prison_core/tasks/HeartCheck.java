...
public class HeartCheck implements Runnable {
			...
			if (Bukkit.getOnlinePlayers().size() > 0) {
				for (Player p : Bukkit.getOnlinePlayers()) {
					if (ItemCheck(p.getInventory().getHelmet())) {
						((CustomTaskEnchant) RealName.HEARTBOOST.getEnchant()).event(p);
					} else {
						p.setMaxHealth(20);
					}
				}
	...
	private boolean ItemCheck(ItemStack i) {
		return i != null && i.hasItemMeta() && i.getItemMeta().hasLore() && i.getItemMeta().getLore().contains(RealName.HEARTBOOST.getEnchant().getName());
	}
}
