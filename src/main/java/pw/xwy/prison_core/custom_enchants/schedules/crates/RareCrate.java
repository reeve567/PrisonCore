...
import pw.xwy.prison_core.custom_enchants.soulcrates.RareSC;
import pw.xwy.prison_core.custom_enchants.utilities.CratesUtility;

import java.util.ArrayList;

public class RareCrate extends BukkitRunnable {
	...
	public RareCrate(ArrayList<Prize> prizes, Inventory inv, Player p, int runs, int rot, boolean log) {
		this.p = p;
		this.prizes = prizes;
		this.inv = inv;
		this.runs = runs;
	...
	public void run() {
		...
		if (!p.isOnline()) {
			if (log) {
				System.out.println(p.getName() + " has left the game during a rare roll.");
			}
			cancel();
		}
		if (moves >= rot) {
